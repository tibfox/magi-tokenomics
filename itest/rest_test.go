...
// LP-reward slice: the same distribution flow as content, run on the distributor's
// `lp` CHANNEL with its own reporter. LP used to be a second deployed contract; it is
// now a channel on the one distributor.
func TestC5LPSlice(t *testing.T) {
	...
	ct.RegisterContract(c5ID, owner, read("../c3-distributor/artifacts/main.wasm"))
	...
	c5init := fmt.Sprintf(`{"token":"%s","kind":"0","funder":"%s","treasury":"hive:treasury","guardianMode":"0","guardianAuth":"hive:guardian","guardianThreshold":"1"}`, tokenID, c2ID)
	call(t, &ct, c5ID, "init", c5init, owner, 0, true)
	call(t, &ct, c5ID, "addChannel", `{"channel":"lp","bucket":"lp","window":"1","reporterMode":"0","reporterAuth":"hive:lpreporter","reporterThreshold":"1","role":"lp"}`, owner, 0, true)
...
// Migration airdrop: fund the contract, airdrop a snapshot, verify balances +
// per-batch idempotency. The airdrop used to be its own contract; it is now part of
// C1, which is why the float has to be transferred in before a batch can pay.
func TestC6MigrationSlice(t *testing.T) {
	...
	t.Cleanup(func() { ct.DataLayer.Stop() })
	ct.RegisterContract(tokenID, owner, read(tokenWasmPath))
	ct.RegisterContract(c6ID, owner, read("../c1-staking/artifacts/main.wasm"))

	call(t, &ct, tokenID, "init", `{"name":"T","symbol":"T","decimals":0,"maxSupply":"1000000000"}`, owner, 0, true)
	call(t, &ct, c6ID, "init", fmt.Sprintf(
		`{"token":"%s","kind":"0","cooldown":"5","epochLen":"1","allow":"","maxAirdrop":"1000000"}`,
